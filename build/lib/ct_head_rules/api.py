...
# --- Short text for summaries ----------------------------------------------


def outcome_text(result: RuleResult) -> str:
...
def summary_reason(result: RuleResult) -> str:
    if result.triggered:
        return "; ".join(c.label for c in result.triggered)
    if result.exclusion_reasons:
        return "; ".join(reason(c) for c in result.exclusion_reasons)
